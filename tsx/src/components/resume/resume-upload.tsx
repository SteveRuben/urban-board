...
import { ResumeUploadProps, ResumeAnalysis } from '@/types/resume';


const ResumeUpload: FC<ResumeUploadProps> = ({ onAnalysisComplete, jobRole }) => {
  ...
  const uploadAndAnalyzeResume = async (): Promise<void> => {
    if (!file) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        // Appel API réel
        const response = await axios.post<ResumeAnalysis>('/api/resumes/analyze', formData, {
          headers: {
  ...
  // Générer une analyse fictive pour le développement
  const generateMockAnalysis = (jobTitle: string): ResumeAnalysis => {
