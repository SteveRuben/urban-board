...
import { Play, Pause, UserPlus, Users, MessageSquare, ChevronRight, Award, Code, Settings, Zap, Camera, CameraOff, Heart, BarChart, Activity, PieChart, FileText, CheckCircle, AlertCircle, LucideIcon } from 'lucide-react';
import { NextPageWithLayout } from '@/types/page';
import Layout from '@/components/layout/layout';

// Types pour les messages et les métriques
...
type MessageSender = 'ai' | 'candidate' | 'recruiter' | 'ai-suggestion' | 'stress-metrics';

interface Message {
  sender: MessageSender;
  content: string | StressMetrics;
}

interface Scenario {
  name: string;
  description: string;
  stressLevel: 'élevé' | 'moyen' | 'faible' | 'très faible';
  autonomous: Message[];
...
const DemoPage: NextPageWithLayout = () => {
  const [activeMode, setActiveMode] = useState<'autonomous' | 'collaborative'>('autonomous');
  const [activeScenario, setActiveScenario] = useState<string>('technical');
  const [demoStep, setDemoStep] = useState<number>(0);
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [candidateCamera, setCandidateCamera] = useState<boolean>(true);
  const [recruiterCamera, setRecruiterCamera] = useState<boolean>(true);
  const [showStressMetrics, setShowStressMetrics] = useState<boolean>(false);
  ...
  const getScenarioMessages = (): Message[] => {
  ...
  const changeMode = (mode: 'autonomous' | 'collaborative'): void => {
    setActiveMode(mode);
    resetDemo();
  };
  
  // Fonction pour changer de scénario
  const changeScenario = (scenario: string): void => {
  ...
  const getLastStressMetrics = (): StressMetrics | null => {
    const messages = getActiveMessages();
    const stressMessages = messages.filter(msg => msg.sender === 'stress-metrics');
    return stressMessages.length > 0 ? stressMessages[stressMessages.length - 1].content as StressMetrics : null;
  };

  // Type guard pour vérifier si le contenu est une chaîne
  const isStringContent = (content: string | StressMetrics): content is string => {
                ...
                {getActiveMessages().filter(msg => msg.sender !== 'stress-metrics').map((message, index) => (
                  <div key={index} className={`flex ${message.sender === 'candidate' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    >
                      ...
                      {isStringContent(message.content) ? message.content : null}
                    </div>
                  </div>
                ))}
                ...
                {getLastStressMetrics() ? (
                  <div className="space-y-6">
                    ...
                    <div className="border-t border-gray-200 pt-4 mt-4">
                      ...
                      {getLastStressMetrics() && getLastStressMetrics()!.stress > 60 ? (
                      ...
                      <div className="flex items-center text-sm mt-1">
                        {getLastStressMetrics() && getLastStressMetrics()!.engagement < 70 ? (
...
DemoPage.getLayout = (page: ReactElement) => <Layout>{page}</Layout>;
