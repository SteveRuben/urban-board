...
const NotificationCenter = () => {
  ...
  useEffect(() => {
    if (user) { // Vérifier que l'utilisateur est connecté
      fetchNotifications();
    }
  }, [filter, user]);
  ...
  const fetchNotifications = async () => {
    if (!user || !user.id) return; // Ne rien faire si l'utilisateur n'est pas connecté
    
    setIsLoading(true);
    try {
      let url = `/api/notifications/?user_id=${user.id}`;
      if (filter !== 'all') {
        url += `&filter=${filter}`;
      }
  ...
  // Si l'utilisateur n'est pas connecté, ne pas afficher le centre de notifications
  if (!user) {
    return null;
  }
