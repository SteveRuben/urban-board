// components/dashboard/DashboardOverviewCard.tsx
...
interface DashboardOverviewCardProps {
  title: string;
  value: string | number;
  icon?: 'total' | 'completed' | 'scheduled' | 'score';
...
// Type pour le résultat du calcul de changement
interface ChangeResult {
  value: string;
  isPositive: boolean;
}

// Type pour les styles de couleur
...
interface ColorStylesMap {
  [key: string]: ColorStyles;
}
...
const DashboardOverviewCard: React.FC<DashboardOverviewCardProps> = ({ 
  ...
  const calculateChange = (): ChangeResult | null => {
    if (!previousValue || previousValue === 0) return null;
    
    const change = ((value as number - previousValue) / previousValue) * 100;
    return {
  ...
  const getChangeColor = (change: ChangeResult | null): string => {
    if (!change) return '';
