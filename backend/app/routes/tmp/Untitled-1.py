...
# Réutiliser le client Redis du module auth_middleware
from .auth_middleware import redis_client
...
def generate_key(key_prefix, user_identifier=None):
    ...
    if user_identifier is None:
        if hasattr(g, 'current_user') and g.current_user:
            ...
            identifier = g.current_user.get('user_id', get_remote_address())
        else:
...
def adaptive_rate_limit(anon_limit, auth_limit, period=60, key_prefix='adaptive'):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if hasattr(g, 'current_user') and g.current_user:
                limit = auth_limit
                key = generate_key(f"{key_prefix}_auth", g.current_user.get('user_id'))
            else:
                limit = anon_limit
